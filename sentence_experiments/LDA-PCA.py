...
def test_germeval(model, tokenizer, subspace_sentences, subspace_labels, n_components, subtask=1):
	...
	pca_results = pca.fit_transform(subspace_sentences)

	lda = LDA(n_components=1)
	lda.fit(pca_results, subspace_labels)

	pca_results_test = pca.transform(germeval_tweets)
...
def test_hasoc(model, tokenizer, subspace_sentences, subspace_labels, n_components, language, subtask=1):
	if language == 'de':
	...
	pca_results_test = pca.transform(hasoc_tweets)
...
def test_arab(model, tokenizer, subspace_sentences, subspace_labels, n_components, subtask=1):
	...
	pca_results_test = pca.transform(arab_tweets)
...
def test_french(model, tokenizer, subspace_sentences, subspace_labels, n_components):
	results = {}
	...
	pca_results_test = pca.transform(french_tweets)
...
def main(sentence_corpus, n_sentencepairs, n_components, normalize):
	...
	with open(sentence_corpus,"r") as f:
		for line in f:
	...
	for a, b in pairs_encoded:
			...
			if normalize:
				center = (a - b)/2
				detached_pairs.append(a - center)
				detached_pairs.append(b - center)
			else:
	...
	#Hasoc german
	results_hasoc_de1 = test_hasoc(model, tokenizer, detached_pairs, detached_labels,n_components=n_components, language='de')
	print("Hasoc German Subtask 1 Test:", results_hasoc_de1)
	#test on Hasoc Subtask 2 (Profanity vs Other)
	results_hasoc_de2 = test_hasoc(model, tokenizer, detached_pairs, detached_labels, n_components=n_components, language='de',subtask=2)
	print("Hasoc German Subtask 2 Test:", results_hasoc_de2)

	#Hasoc english
	results_hasoc_en1 = test_hasoc(model, tokenizer, detached_pairs, detached_labels,n_components=n_components, language='en')
	print("Hasoc English Subtask 1 Test:", results_hasoc_en1)
	#test on Hasoc Subtask 2 (Profanity vs Other)
	results_hasoc_en2 = test_hasoc(model, tokenizer, detached_pairs, detached_labels, n_components=n_components, language='en',subtask=2)
	print("Hasoc English Subtask 2 Test:", results_hasoc_en2)

	#Arab
	results_arab1 = test_arab(model, tokenizer, detached_pairs, detached_labels, n_components=n_components)
	print("Arabic Abusive Lang. Detection Subtask 1 Test:", results_arab1)
	#test on Arab Subtask 2 (Profanity vs Other)
	results_arab2 = test_arab(model, tokenizer, detached_pairs, detached_labels, n_components=n_components, subtask=2)
	...
	return results_hasoc_de1, results_hasoc_de2, results_hasoc_en1, results_hasoc_en2, results_arab1, results_arab2, results_french1
...
if __name__ == "__main__":
	...
	parser.add_argument("-ar", "--arab", required=True, dest="arab_path",
						help="Specify path to Arabic Abusive Lang. Detection  test set")

	# Number of sentence pairs to build subspace on
	parser.add_argument("-s", "--sentences", required=True, dest="n_sentences", type=int, help="specify number of sentences to apply PCA on from 1 - 200", choices=list(range(1,201)))

	# Number of principal components in PCA
	parser.add_argument("-pc", "--pcs", required=True, dest="n_components", type=int, help="specify number of PCA components from 1 - 200", choices=list(range(1,201)))

	# Normalisation
	parser.add_argument("-norm", "--norm", dest="normalize", type=bool, help="specify whether to apply normalisation or not", choices=[True, False])
	...
	main(args.sentence_corpus, args.n_sentences, args.n_components, args.normalize)
